...
### 有限状态机支持
- **简化语法**：基于宏定义的线性状态机语法糖，无需指定状态机名称
- **非阻塞等待**：内置fsm_wait支持，使用巧妙的临时状态机制
- **线性结构**：易于理解和维护的状态转换逻辑
- **自动管理**：自动维护状态变量和时间戳，无需手动声明
...
void task_fsm(void)
{
    ...
    fsm_wait(300);  // 非阻塞等待300毫秒
    sl_printf("wait 500ms");
    fsm_wait(500);  // 非阻塞等待500毫秒
    sl_printf("wait 1000ms");
    fsm_wait(1000); // 非阻塞等待1000毫秒
    fsm_goto(0);    // 循环回到状态0
    _CASE_END
